...
\chapter{Introduction}
	...
	The first one to bring the concept of robotics at micro scale, especially for medical applications into a scientific context was Richard Feynman in the famous 1959 CalTech conference \citetitle{Feynman} \cite{Feynman}, from which the following quote is taken:
	\begin{displayquote}
	...
	\item no tether
	\end{itemize}
	and its design needs some key ingredients, such as an actuation to make it motile, some kind of control and a power source to be able to perform desired tasks, as well as some sensing capability, to sample and observe surrounding environment \cite{palagi_bioinspired_2018}. 
	...
	In particular, the most studied Janus Sphere in this field is a \ch{SiO2} sphere where \ch{Pt} is deposed using a sputtering machine to coat just one hemisphere; the fuel is \ch{H2O2} that is decomposed in 
	\begin{align}
	...
	\subsection{Objective 2: Analysis} %\todo{nell'analisi c'è anche tutta la parte di analisi quantitativa delle simulazioni... e forse l'inferenza la lascerei a parte}
	...
	%\todo{Questo paragrafo non dice niente... piuttosto fai solo un breve cappello introduttivo su cos'è la materia attiva citando qualche articolo o review di riferimento.}
	...
	In these works, the problem of relating theoretical models and reality is tackled analyzing videos of starling swarms in order to understand characteristics of interactions. %\todo{non capisco questa frase}
	...
	\begin{figure}[htp]
		\centering
		\subfloat[][Phase transition behavior of polarization and its susceptivity]{\includegraphics[width=\subfigwidth]{martin_phasetransition.png}}\quad
	...
	\cite{singh_pair_2024} studies what happens when two \ch{SiO2-Pt} Janus particles come together in different configurations; authors model a collision analyzing the overlap between concentration fields around the two particles as well as the torque caused by the interaction between solvent flows around each hemisphere of a pair of Janus microswimmers.%\todo{in che senso ne tengono conto?} 
	...
	The main fact, supported both by simulations and experimental evidence, to take away is that chemical and hydrodynamic interactions do not cause reorientation for pairs of particles separated by a long distance, and that leading effects are short-range chemical torques.%\todo{frase non chiara, riscrivere}
	...
	\begin{figure}[htp]
		\centering
		\subfloat[]{\includegraphics[width=\subfigwidth]{bag1_1.png}}
		\subfloat[]{\includegraphics[width=\subfigwidth]{bag1_2.png}}\\
		\subfloat[]{\includegraphics[width=\subfigwidth]{bag1_3.png}}
		
		\caption{Potential shapes for different values of parameters \cite{bag_interaction_2021}.}
		\label{fig:bag1}
	\end{figure}
	...
	\subsection[ABP simulations]{Numerical Simulations of active particles}%\todo{in che senso "physics"?}
		...
		For Heun error does not depend on the value of potential and its derivatives but only on integration step, making it possible to get more accurate estimates of the equilibrium distribution.%\todo{quindi? spiega anche per qualcuno che non è né te né Mannella :)}
