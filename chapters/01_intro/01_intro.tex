...
\chapter{Introduction}
	...
	One of the most studied Janus mircoparticle is \ch{SiO2-Pt}, where \ch{Pt} is deposed on one hemisphere of the particle using a sputtering machine; the fuel is \ch{H2O2} that is decomposed in 
	\begin{align}
	...
	Dispersing Janus colloids in a \ch{H2O2} solution in a quasi-2D environment, the types of configurations particles can scatter in are limited and categorized by \citeauthor{singh_pair_2024} in four dynamical states. 
	...
	In terms of machine learning model, the simplest approach is using some global attribute of the active particle ensemble to predict the potential via a Deep Neural Network (DNN). %\todo{spiega meglio}
	...
	\subsection[ABP simulations]{Numerical Simulations of active particles}%\todo{in che senso "physics"?}
		...
		\begin{equation}
			\begin{cases}
				\dd{x} = v \cos{\theta} \dd{t} + \sqrt{2D_t}\dd{W_x}, \\
				\dd{y} = v \sin{\theta}\dd{t} + \sqrt{2D_t}\dd{W_y}, \\
		...
		\begin{equation} \label{eq:fin_diff1}
			\begin{cases}
				...
				\theta_{n+1} = \theta_n + \omega \Delta t + \sqrt{2D_r \Delta t} \, W_{\theta,n}\\
			\end{cases}
		\end{equation}
		where the case of a deterministic self-propulsion angular velocity $\omega$ is also taken into account \cite{callegari_numerical_2019}.
		...
		\begin{equation} \label{eq:fin_diff_with_forces}
			\begin{cases}
				...
				\theta_{n+1} = \theta_n +\left(\omega + \frac{D_r}{k_B T}{}T_{ext}\right) \Delta t + \sqrt{2D_r \Delta t} \, W_{\theta,n}\\
			\end{cases}
		\end{equation}
