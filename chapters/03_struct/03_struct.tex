...
\begin{document}
	...
	Our investigation in structure and dynamics is based upon a small yet descriptive set of tools that help unveiling how particles spatially organise and how they move.
		...
		We have $k = 1.$ and the range of interaction is \SI{5}{\micro\meter}, which, being $2.5R$, with $R$ particle radius, is enough to make particles interact only with one layer of particles around them, simulating a nearest-neighbor interaction.
		...
		Every \num{100} steps, global polarization is computed and stored in a vector that, from now on, will be called the \emph{polarization history} vector.
		...
		For every value of $\alpha$, polarization is taken as the mean of the polarization history vector, with thermalization cut off, while we computed susceptivity using both a blocking and a K
		...
		To implement, different velocities we used normal distribution at mean velocity 10um/s while a changing
		...
		A remarkable observation is that although alignment and clustering are often related, here a drastic downfall of clustering does not reflect in a big difference in polarization, resulting in a sparse system with a high alignment.
