...
\begin{document}
	...
	Our investigation in structure and dynamics is based upon a small yet descriptive set of tools that help unveiling how particles spatially organise and how they move {\color{blue} insert citation about 1994 Nobel, "where atoms are, what atoms do"}.
		...
		In order to compute $g(r)$ in our simulations, we first need to take distances between all pairs of particles (counting twice the same couple\todo{che intendi?}) accounting for periodic boundary conditions as any particle became the center of the system, similarly to the periodic interaction method in section \ref{intrange}. 
		...
		In Figure \ref{fig:flock40}\todo{casino con la numerazione - forse problemi con il ContinuedFloat}, an example of how flocking transition happens with $k = 10$, with an interaction range of \SI{40}{\micro\meter}.
		% \begin{figure}[htp]
		% 	\centering
		...
		% 	\label{fig:flock40}
		% \end{figure}
		\todo{perché hai spezzato questa figura in 2? La numerazione della figura è sbagliata. Ho commentato le due figure e ne ho fatta una unica (risolvendo anche il problema numerazione)... vedi se può andare. I numeri sugli assi sono illeggibili, tanto vale non metterli proprio.}
		...
		\todo{anche qui un grafico di forza vs. distanza sarebbe utile; spiega perché hai scelto questi valori fissi per il potenziale; il range di interazione quant'è?}
		...
		An active speed makes the system non-isotropic, giving a preferred direction to particles, so we also investigated how velocity interacts with the sign of the directional coupling parameter $\alpha$, repeating the analysis for positive and negative values.
		...
		\subsection{Velocity Distribution}\todo{quant'è $alpha$?}
		A simple way to add some complexity into the model is to generate particles with different velocities, initializing them with their self-propulsion speed constant but sampled from some distribution. 
		...
		\begin{figure}\ContinuedFloat
			...
			\label{fig:lj_oc_rdf}
		\end{figure}
		\todo{RDF: riunire figura}
	
		
		\subsection{Angular Velocity}\todo{quant'è $alpha$?}
	...
	\todo{commento sui comportamenti attesi da un sistema con parametri simili a quelli reali}
