...
\label{chap:int_impl}
	...
	Here, we look into a qualitative comparison between the simulated behaviors and the observed ones, focusing in this case on interactions between pairs or small groups of particles.
	...
	Still, observing how single particles interact with clusters gives some insights on contact behaviors, making particle clumps' presence useful.
	In principle, it is difficult to create stable clusters in simulations, since our framework does not involve the possibility of physically attached particle, but an attractive potential is needed to mimic observed behaviors.
	
	Due to differences and irregularities in the fabrication process, not all particles have the same self-propulsion velocity.
	In simulations we can reproduce this generating particles' velocity from a distribution.
	Although particles orientation is difficult to measure in our experimental videos, it is possible to note that some particles are spinning around and some others have both a linear and a rotational self-propulsion, in a Chiral Brownian Particles fashion, \cite{callegari_numerical_2019, bechinger_active_2016}.
	
	With all of this said, the ingredients for a simulation that reproduces experimental behaviors are: a short-range potential with an attractive part, a linear and angular velocity distribution.
	...
	To catch this asimmetry, we generated simulated particles' velocities from an exponential distribution; this is done to maximize the number of collisions as well.
